...
# -----------------------------------------------------------------------------
# This is the shared infrastructure stack that can be instantiated per environment.
# All environment-specific configuration is passed through these variables.
# -----------------------------------------------------------------------------
...
variable "region" {
  ...
  validation {
    ...
    error_message = "Region must be a valid AWS region format (e.g., eu-north-1)."
  }
}
...
variable "admin_usernames" {
  ...
  validation {
    condition     = alltrue([for u in var.admin_usernames : can(regex("^[a-zA-Z0-9_+=,.@-]+$", u))])
    error_message = "All admin_usernames must be valid IAM username format."
  }
}
...
variable "public_access_cidrs" {
  description = "CIDR blocks allowed to access the EKS API endpoint. Restrict for security."
  type        = list(string)
  default     = []  # Empty means no public access - most secure
...
variable "db_password" {
  description = "Master password for the database. If empty, will try to use GitHub secret."
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "grafana_admin_password" {
  description = "Grafana admin password. If empty, will try to use GitHub secret."
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "github_backend_token" {
  description = "GitHub token for backend repository"
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "github_frontend_token" {
  description = "GitHub token for frontend repository"
  type        = string
  default     = ""
  sensitive   = true
}
